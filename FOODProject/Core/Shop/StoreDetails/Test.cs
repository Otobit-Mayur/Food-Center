        ...
        public Result AddStoreDetails(Model.Shop.ShopDetail.ShopDetail value)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                using (FoodCenterDataContext context = new FoodCenterDataContext())
                {
                    ...
                    var ci1 = context.Categories.SingleOrDefault(c => c.CategoryName == value.Category.String).CategoryId;

                    var ui1 = context.Users.SingleOrDefault(c => c.EmailId == value.User.String);
                    var check1 = context.ShopDetails.SingleOrDefault(c => c.UserId == ui.UserId);
                    if (check1 != null)
                    {
                        throw new ArgumentException("Entered Email Already Registered for another store");
                    }
